...
int main()
{
    ...
    scanf("%d",&n);
    int sum_tat=0,sum_wt=0,sum_rt=0,total_idle_time=0,prev=0,length_cycle; //prev stores the completion time of just previous process
    ...
    while(completed!=n)
    {
        ...
        int min_index = -1; // Gives the process having least brust time
        int minimum = INT_MAX; //stores the actual cpu brust time of above
        for(int i = 0; i < n; i++) {
